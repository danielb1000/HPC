...
def calcular_aceleracao(pos_x, pos_y):
    ...
    ax = -pos_x / distancia_cubo
    ay = -pos_y / distancia_cubo
    return ax, ay
...
pos_ec_y = 0.0
vel_ec_x = 0.0
vel_ec_y = 1.0  # Para o Euler-Cromer
...
for _ in range(passos):
    ...
    pos_euler_x = pos_euler_x + vel_euler_x * dt
    pos_euler_y = pos_euler_y + vel_euler_y * dt
    vel_euler_x = vel_euler_x + acel_euler_x * dt
    vel_euler_y = vel_euler_y + acel_euler_y * dt
    ...
    vel_ec_x = vel_ec_x + acel_ec_x * dt
    vel_ec_y = vel_ec_y + acel_ec_y * dt
    # E de seguida usa a NOVA velocidade para atualizar a posição
    pos_ec_x = pos_ec_x + vel_ec_x * dt
    pos_ec_y = pos_ec_y + vel_ec_y * dt
    

    # --- 3. MÉTODO VELOCITY VERLET ---
    # Passo 1: Atualizar posição com a velocidade e aceleração atuais
    pos_verlet_x = pos_verlet_x + vel_verlet_x * dt + 0.5 * acel_verlet_x * (dt**2)
    pos_verlet_y = pos_verlet_y + vel_verlet_y * dt + 0.5 * acel_verlet_y * (dt**2)
    ...
    vel_verlet_x = vel_verlet_x + 0.5 * (acel_verlet_x + nova_acel_verlet_x) * dt
    vel_verlet_y = vel_verlet_y + 0.5 * (acel_verlet_y + nova_acel_verlet_y) * dt
    
    # Atualizar a aceleração para o ciclo seguinte
    acel_verlet_x = nova_acel_verlet_x
    acel_verlet_y = nova_acel_verlet_y
