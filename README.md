...
Para correr o projeto, necessitas das seguintes dependências:

```bash
pip install numpy matplotlib pycuda
```
*(Nota: A biblioteca `pycuda` exige que o sistema tenha o **CUDA Toolkit** instalado e uma placa gráfica NVIDIA compatível).*

## 🏃 Como Executar

Para iniciar o benchmark, basta correr o script principal. Este irá testar primeiro o algoritmo no CPU, depois no GPU e, por fim, apresentará a diferença de performance:
